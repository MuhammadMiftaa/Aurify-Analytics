...
const getUserTransactions: grpc.handleUnaryCall<any, any> = async (
  call,
  callback,
) => {
  try {
    ...
    if (grpcDateOption) {
      ...
      if (grpcRange && grpcRange.getStart()) {
        dateOption.range = {
...
const getUserBalance: grpc.handleUnaryCall<any, any> = async (
  call,
  callback,
) => {
  try {
...
const getUserFinancialSummary: grpc.handleUnaryCall<any, any> = async (
  call,
  callback,
) => {
  try {
    ...
    const summaries = (result || []).map((s: any) => {
      ...
      if (s.InvestmentSummary) {
        ...
        inv.setInvestmentGrowthPct(
          s.InvestmentSummary.InvestmentGrowthPct || 0,
        );
...
const getUserNetWorthComposition: grpc.handleUnaryCall<any, any> = async (
  call,
  callback,
) => {
  try {
...
const getUserWallets: grpc.handleUnaryCall<any, any> = async (
  call,
  callback,
) => {
  try {
